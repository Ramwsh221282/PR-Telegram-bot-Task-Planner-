...
public sealed class NotificationsReadableRepository : INotificationsReadableRepository
{
    ...
    {
        const string sql = $"SELECT receiver_name FROM notification_receivers WHERE receiver_id = @id";
        ...
        return string.IsNullOrWhiteSpace(name) ?
            Result.Failure<NotificationReceiverName>($"Не найден чат с ID: {id}") :
            NotificationReceiverName.Create(name).Value;
    }
}
